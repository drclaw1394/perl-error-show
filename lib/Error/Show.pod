...
Honours perl -c, -w, -W and  -X flags.  To only perform a syntax check, use the
-c flag like normal.  To enable showing context for warnings during a check,
use -w -W  or -X flags.

	perl -MError::Show -c examples/file_syn_error1.pl 
	perl -MError::Show -c -w examples/file_syn_error1.pl 

Also honours -I flags to allow user include paths:
...
display the code context of the first error.
...
The -c -w -X -W -I flags are honoured when the script is being checked
internally, to give as transparent as possible script checking before
executing.
