...
from the command line or as a debugging tool. 


Consider the following code (t/require.t.pl):
...
Working on multiple modules which depend on each other makes traking down
syntax errors painful. While there is always a bunch of line numbers indicating
where the error might have come from, it hardly triggers any visions of the
code. That is unless you just edited the file 10s previously or have an
impecible memory.
